...
func TestAccPackerChannelAssignment_SimpleSetUnset(t *testing.T) {
	bucketSlug := testhelpers.CreateTestSlug(t)
...
func TestAccPackerChannelAssignment_AssignLatest(t *testing.T) {
	...
	generateStep := func(iterationData, channelResource, assignmentResource testhelpers.ConfigBuilder) resource.TestStep {
		return resource.TestStep{
			Config: testhelpers.BuildTestConfig(iterationData, channelResource, assignmentResource),
			Check: resource.ComposeAggregateTestCheckFunc(
...
func TestAccPackerChannelAssignment_InvalidInputs(t *testing.T) {
	...
	generateStep := func(iterID string, iterFingerprint string, iterVersion string, errorRegex string) resource.TestStep {
		return resource.TestStep{
			Config: testhelpers.BuildTestConfig(testAccPackerAssignmentBuilder(
				"InvalidInputs",
				fmt.Sprintf("%q", bucketSlug),
				fmt.Sprintf("%q", channelSlug),
...
func TestAccPackerChannelAssignment_CreateFailsWhenPreassigned(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testhelpers.BuildTestConfig(channel),
			},
			{
				PreConfig: func() {
				...
				Config:      testhelpers.BuildTestConfig(channel, assignment),
...
func TestAccPackerChannelAssignment_HCPManagedChannelErrors(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config:      testhelpers.BuildTestConfig(assignment),
			...
			{ // Create a dummy non-empty state so that `CheckDestroy` will run.
				Config: testhelpers.DummyNonemptyStateConfig(),
			},
		},
	})
}
...
func testAccPackerDataIterationBuilder(uniqueName string, bucketName string, channelName string) testhelpers.ConfigBuilder {
...
func testAccPackerChannelBuilder(uniqueName string, channelName string, bucketName string) testhelpers.ConfigBuilder {
...
func testAccPackerAssignmentBuilderBase(uniqueName string, bucketName string, channelName string) testhelpers.ConfigBuilder {
...
func testAccPackerAssignmentBuilderBaseWithChannelReference(uniqueName string, channel testhelpers.ConfigBuilder) testhelpers.ConfigBuilder {
...
func testAccPackerAssignmentBuilder(uniqueName string, bucketName string, channelName string, iterID string, iterFingerprint string, iterVersion string) testhelpers.ConfigBuilder {
	return testhelpers.NewResourceConfigBuilder(
...
func testAccPackerAssignmentBuilderFromAssignment(oldAssignment testhelpers.ConfigBuilder, iterID string, iterFingerprint string, iterVersion string) testhelpers.ConfigBuilder {
	return testAccPackerAssignmentBuilder(
...
func testAccPackerAssignmentBuilderWithChannelReference(uniqueName string, channel testhelpers.ConfigBuilder, iterID string, iterFingerprint string, iterVersion string) testhelpers.ConfigBuilder {
	return testAccPackerAssignmentBuilder(
		uniqueName,
		channel.AttributeRef("bucket_name"),
		channel.AttributeRef("name"),
...
func pullIterationFromAPIWithAssignmentState(resourceName string, state *terraform.State) (*models.HashicorpCloudPackerIteration, error) {
	...
	bucketName, err := testhelpers.GetAttributeFromResourceInState(resourceName, "bucket_name", state)
	if err != nil {
		return nil, err
	}
	channelName, err := testhelpers.GetAttributeFromResourceInState(resourceName, "channel_name", state)
	if err != nil {
		return nil, err
	}
