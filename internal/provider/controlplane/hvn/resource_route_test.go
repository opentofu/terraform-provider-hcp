...
func TestAccHvnRoute(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		PreCheck:          func() { testhelpers.PreCheck(t, map[string]bool{"aws": true, "azure": false}) },
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testhelpers.TestConfig(testAccHvnRouteConfig),
				Check: resource.ComposeTestCheckFunc(
					...
					testhelpers.CheckLinkFromState(resourceName, "self_link", hvnRouteUniqueAWSName, links.HVNRouteResourceType, "hcp_hvn.test"),
					testhelpers.CheckLinkFromState(resourceName, "target_link", hvnRouteUniqueAWSName, links.PeeringResourceType, "hcp_hvn.test"),
				),
			},
...
func testAccCheckHvnRouteExists(name string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		hvnRouteLink, err := links.BuildLinkFromURL(id, links.HVNRouteResourceType, client.Config.OrganizationID)
		if err != nil {
		...
		hvnLink, err := links.ParseLinkURL(hvnURL, links.HvnResourceType)
		if err != nil {
