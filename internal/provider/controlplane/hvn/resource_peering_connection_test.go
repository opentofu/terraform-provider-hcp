...
func TestAccHvnPeeringConnection(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Tests create
			{
				Config: testhelpers.TestConfig(testAccHvnPeeringConnectionConfig),
				Check: resource.ComposeTestCheckFunc(
					...
					testhelpers.CheckLinkFromState(resourceName, "hvn_1", hvn1UniqueID, links.HvnResourceType, resourceName),
					testhelpers.CheckLinkFromState(resourceName, "hvn_2", hvn2UniqueID, links.HvnResourceType, resourceName),
				),
			},
			// Tests import
			{
			...
			// Tests datasource
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testhelpers.CheckLinkFromState(dataSourceName, "hvn_1", hvn1UniqueID, links.HvnResourceType, dataSourceName),
					testhelpers.CheckLinkFromState(dataSourceName, "hvn_2", hvn2UniqueID, links.HvnResourceType, dataSourceName),
				),
			},
		},
	})
...
func testAccCheckHvnPeeringConnectionExists(name string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		peeringLink, err := links.BuildLinkFromURL(id, links.PeeringResourceType, client.Config.OrganizationID)
		if err != nil {
		...
		hvnLink, err := links.BuildLinkFromURL(hvnSelfLink, links.HvnResourceType, client.Config.OrganizationID)
		if err != nil {
...
func testAccCheckHvnPeeringConnectionDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		case "hcp_hvn_peering_connection":
			...
			hvnLink, err := links.BuildLinkFromURL(rs.Primary.Attributes["hvn_1"], links.HvnResourceType, client.Config.OrganizationID)
			if err != nil {
				return fmt.Errorf("unable to parse hvn_1 link URL for %q: %v", id, err)
			}
